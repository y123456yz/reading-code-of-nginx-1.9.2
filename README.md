...
16.4.3
	重新分析写超时定时器原理，生效方法，
	结合写超时定时器重新分析后端服务器拔掉网线或者直接断电情况下的bug问题，参考ngx_http_upstream_send_request
...
改造点及可疑问题:
	...
	如果发往后端服务器数据大小很大，可能需要多次write，这时候会由write timeout来判断出后端出现问题。但是如果发往后端数据长度小，则不会添加write timeout定时器，也
	就是永远判断不出后端服务器不在线。见ngx_http_upstream_send_request
