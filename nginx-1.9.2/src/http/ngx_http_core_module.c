...
因为server类配置可能同时出现在main中，所以需要存储这写配置，所以要创建src来存储他们，就是上面的sss配置。
...
static ngx_http_module_t  ngx_http_core_module_ctx = {
    ...
    ////解析到http{}行时，在ngx_http_block执行。该函数创建的结构体成员只会出现在http中，不会出现在server和location中
...
/*
NGX_HTTP_REWRITE_PHASE  NGX_HTTP_POST_REWRITE_PHASE阶段的checker方法是ngx_http_core_rewrite_phase。表10-2总结了该阶段下ngx_http_handler_pt处理方法的返回值是如何影响HTTP框
架执行的，注意，这个阶段中不存在返回值可以使请求直接跳到下一个阶段执行。
...
ngx_http_request_t *r是当前的请求，也就是父请求。
    (2) uri
    ngx_str_t *uri是子请求的URI，它对究竞选用nginx.conf配置文件中的哪个模块来处理
子请求起决定性作用。
    (3) ngx_str_t *args
    ngx_str_t *args是子请求的URI参数，如果没有参数，可以传送NULL空指针。
    (4) ngx_http_request_t **psr
    psr是输出参数而不是输入参数，它将把ngx_http_subrequest生成的子请求传出来。一
...
SUBREQUEST_IN_MEMORY。这个宏会将子请求的subrequest_in_memory标志位置为1，
这意味着如果子请求使用upstream访问上游服务器，那么上游服务器的响应都将会在内存中
处理；③NGX- HTTP—SUBREQUEST—WAITED。这个宏会将子请求的waited标志位置为1，
当子请求提前结束时，有个done标志位会置为1，但目前HTTP框架并没有针对这两个标志
位做任何实质性处理。注意，flag是按比特位操作的，这样可以同时含有上述3个值。
    (7)返回值
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    ctx->srv_conf = pctx->srv_conf; //指向父的srv
  ...
  # 但是所有 /images/ 目录的请求将由 [Configuration C]处理.   
    ...
    */
    if (cf->args->nelts == 3) {
        ...
        //location / {}全部都匹配，   //location @mytest {}  //location !~ mytest {}  //location !~* mytest {}
