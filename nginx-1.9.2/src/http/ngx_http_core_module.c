...
ÈçÐè×ªÔØÇë×¢Ã÷³ö´¦£º  http://www.ttlsa.com/html/2907.html
    */
    ...
    ÅäÖÃ¿é£ºlocation
    NginxÍ¨¹ýlimit_exceptºóÃæÖ¸¶¨µÄ·½·¨ÃûÀ´ÏÞÖÆÓÃ»§ÇëÇó¡£·½·¨Ãû¿ÉÈ¡Öµ°üÀ¨£ºGET¡¢HEAD¡¢POST¡¢PUT¡¢DELETE¡¢MKCOL¡¢COPY¡¢MOVE¡¢OPTIONS¡¢PROPFIND¡¢PROPPATCH¡¢LOCK¡¢UNLOCK»òÕßPATCH¡£ÀýÈç£º
    limit_except GET {
        allow 192.168.1.0/32;
        deny  all;
    }
    
    ×¢Òâ£¬ÔÊÐíGET·½·¨¾ÍÒâÎ¶×ÅÒ²ÔÊÐíHEAD·½·¨¡£Òò´Ë£¬ÉÏÃæÕâ¶Î´úÂë±íÊ¾µÄÊÇ½ûÖ¹GET·½·¨ºÍHEAD·½·¨£¬µ«ÆäËûHTTP·½·¨ÊÇÔÊÐíµÄ¡£
    */
...
*/ 
    ...
    Èç¹ûaio on; sendfile¶¼ÅäÖÃÁË£¬Ôòµ±ÎÄ¼þ´óÐ¡´óÓÚµÈÓÚdirectioÖ¸¶¨size(Ä¬ÈÏ512)µÄÊ±ºòÊ¹ÓÃaio,µ±Ð¡ÓÚsize»òÕßdirectio offµÄÊ±ºòÊ¹ÓÃsendfile
    ÉúÐ§¼ûngx_open_and_stat_file  if (of->directio <= ngx_file_size(&fi)) { ngx_directio_on }
    */ //ngx_output_chain_as_is  ngx_output_chain_copy_bufÊÇaioºÍsendfileºÍÆÕÍ¨ÎÄ¼þ¶ÁÐ´µÄ·ÖÖ§µã
    { ngx_string("sendfile"),
      NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF
                        |NGX_CONF_FLAG,
      ngx_conf_set_flag_slot,
...
Ä¬ÈÏ£ºdirectio off;
ÅäÖÃ¿é£ºhttp¡¢server¡¢location
´ËÅäÖÃÏîÔÚFreeBSDºÍLinuxÏµÍ³ÉÏÊ¹ÓÃO_DIRECTÑ¡ÏîÈ¥¶ÁÈ¡ÎÄ¼þ£¬»º³åÇø´óÐ¡Îªsize£¬Í¨³£¶Ô´óÎÄ¼þµÄ¶ÁÈ¡ËÙ¶ÈÓÐÓÅ»¯×÷ÓÃ¡£×¢Òâ£¬ËüÓësendfile¹¦ÄÜÊÇ»¥³âµÄ¡£
*/
/*
When both AIO and sendfile are enabled on Linux, AIO is used for files that are larger than or equal to the size specified in the 
directio directive, while sendfile is used for files of smaller sizes or when directio is disabled. 
...
´ËÅäÖÃÊÇ¶Ô¿Í»§¶ËÇëÇóÏÞÖÆÃ¿Ãë´«ÊäµÄ×Ö½ÚÊý¡£speed¿ÉÒÔÊ¹ÓÃ2.2.4½ÚÖÐÌáµ½µÄ¶àÖÖµ¥Î»£¬Ä¬ÈÏ²ÎÊýÎª0£¬±íÊ¾²»ÏÞËÙ¡£

Õë¶Ô²»Í¬µÄ¿Í»§¶Ë£¬¿ÉÒÔÓÃ$ limit_rate²ÎÊýÖ´ÐÐ²»Í¬µÄÏÞËÙ²ßÂÔ¡£ÀýÈç£º
server {
 if ($slow) {
  set $limit_rate  4k;
 }
...
ÅäÖÃ¿é£ºhttp¡¢server¡¢location¡¢if
´ËÅäÖÃ±íÊ¾NginxÏò¿Í»§¶Ë·¢ËÍµÄÏìÓ¦³¤¶È³¬¹ýlimit_rate_afterºó²Å¿ªÊ¼ÏÞËÙ¡£ÀýÈç£º
limit_rate_after 1m;
limit_rate 100k;

11.9.2½Ú½«´ÓÔ´ÂëÉÏ½éÉÜlimit_rate_afterÓëlimit_rateµÄÇø±ð£¬ÒÔ¼°HTTP¿ò¼ÜÊÇÈçºÎÊ¹ÓÃËüÃÇÀ´ÏÞÖÆ·¢ËÍÏìÓ¦ËÙ¶ÈµÄ¡£
*/
...
ÅäÖÃ¿é£ºhttp¡¢server¡¢location
Ä¬ÈÏÎªÃ¿60Ãë¼ì²éÒ»´Î»º´æÖÐµÄÔªËØÊÇ·ñÈÔÓÐÐ§¡£
*/
...
ÅäÖÃ¿é£ºhttp¡¢server¡¢location
ËüÓëopen_file_cacheÖÐµÄinactive²ÎÊýÅäºÏÊ¹ÓÃ¡£Èç¹ûÔÚinactiveÖ¸¶¨µÄÊ±¼ä¶ÎÄÚ£¬·ÃÎÊ´ÎÊý³¬¹ýÁËopen_file_cache_min_usesÖ¸¶¨µÄ×îÐ¡´ÎÊý£¬ÄÇÃ´½«²»»á±»ÌÔÌ­³ö»º´æ¡£
*/
 ...
 */
    if (!r->internal) {
       ...
       µÄµÚÒ»¸ö»Øµ÷·½·¨¿ªÊ¼Ö´ÐÐ£¨²Î¼û10.6½Ú£¬ÁË½ângx_http_phase_engine_tÊÇÈçºÎ½«¸÷HTTPÄ£¿éµÄ»Øµ÷·½·¨¹¹Ôì³ÉhandlersÊý×éµÄ£©¡£
          */
        r->phase_handler = 0;
...
ÔÚÏòheadersÁ´±íÖÐÌí¼Ó×Ô¶¨ÒåµÄHTTPÍ·²¿Ê±£¬¿ÉÒÔ²Î¿¼3.2.3½ÚÖÐngx_list_pushµÄÊ¹ÓÃ·½·¨¡£ÕâÀïÓÐÒ»¸ö¼òµ¥µÄÀý×Ó£¬ÈçÏÂËùÊ¾¡£
...
http_subrequest·½·¨ÖÐ£¬Èç¹ûngx_http_subrequest·µ»Ø³É¹¦£¬psr¾ÍÖ¸Ïò½¨Á¢ºÃµÄ×ÓÇëÇó¡£
    ...
    ÕâÀï´«ÈË5.4.2½ÚÖÐ´´½¨µÄngx_http_post_subrequest_t½á¹¹ÌåµØÖ·£¬ËüÖ¸³ö×ÓÇëÇó½áÊø
Ê±±ØÐë»Øµ÷µÄ´¦Àí·½·¨¡£
    (6) ngx_uint_t flags
