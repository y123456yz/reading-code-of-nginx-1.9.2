...
struct ngx_http_upstream_s { //该结构中的部分成员是从upstream{}中的相关配置里面(ngx_http_upstream_conf_t)获取的
     ...
     */
    ngx_event_pipe_t                *pipe; //ngx_http_fastcgi_handler中创建空间
...
由ngx_http_upstream_conf_t结构体中的buffer_size成员决定
    */ //ngx_http_upstream_process_header中创建空间和赋值，通过该buf接受recv后端数据
...
*/ //ngx_http_fastcgi_process_header(ngx_http_XXX_process_header) //在ngx_http_upstream_process_header中执行
...
*/ //销毁upstream请求时调用  ngx_http_XXX_finalize_request  //在ngx_http_upstream_finalize_request中执行
...
//制定了存储前端文件的路径，参数on指定了将使用root和alias指令相同的路径，off禁止存储，此外，参数中可以使用变量使路径名更明确：fastcgi_store /data/www$original_uri;
    ...
    */ //fastcgi赋值见ngx_http_fastcgi_handler u->buffering = flcf->upstream.buffering; //见xxx_buffering如fastcgi_buffering  是否换成后端服务器应答回来的包体
