...
typedef struct { 
    ...
    读者可以先回顾一下ngx_list_t链表和ngx_table_elt_t结构体的用法。前面3.2.3节中已经介绍过，headers是一个ngx_list_t链表，它存储
    着解析过的所有HTTP头部，链表中的元素都是ngx_table_elt_t类型。下面尝试在一个用户请求中找到“Rpc-Description”头部，首先判断其值
    是否为“uploadFile”，再决定后续的服务器行为，代码如下。
...
*/
    ...
    每一个元素都是3.2.4节介绍过的ngx_table_elt_t成员*/
    ...
    */
    ...
    //解析完头部行后通过ngx_http_process_request_header来开辟空间从而来存储请求体中的内容
...
typedef struct {  //获取请求对应的server块信息，server_name信息可以参考ngx_http_set_virtual_server作为例子
    ...
    ngx_http_addr_conf_t             *addr_conf; //赋值见ngx_http_init_connection  这里面存储serv loc等配置信息以及server_name配置信息
    
    //客户端连接服务端accept成功后，该accept对应的listen所在的server{}配置块所在的ctx上下文//赋值见ngx_http_init_connection
...
*/
typedef struct {
    ngx_http_post_subrequest_pt       handler; //在函数ngx_http_finalize_request中执行
    void                             *data;
...
/*
当派生一个子请求访问第三方服务附，如果只是希望接收到完整的响应后在Nginx中解析、处理，那么这里就不需要postpone模块，就像5.6节中
的例子那样处理即可；如果原始请求派生出许多子请求，并且希望将所有子请求的响应依次转发给客户端，当然，这里的“依次”就是按照创建
...
ngx_http_postpone_filter_module过滤模块处理这段要发送的包体。下面看一下过滤包体的ngx_http_postpone_filter方法（在阅读完第11章后再
回头看这段代码，概念可能会更加清晰）：
...
ngx_http_request_t结构体的内容很多，本节不会探讨ngx_http_request_t中所有成员的意义（ngx_http_request_t结构体中的许多成员只有HTTP框架才感兴趣，
在11.3.1节会更详细的说明），
...
struct ngx_http_request_s { //当接收到客户端请求数据后，调用ngx_http_create_request中创建并赋值
    ...
    /*
    ctx与4.3.2节中ngx_http_conf_ctxt结构的3个数组成员非常相似，它们都
    ...
    */
     ...
     */
    ngx_pool_t                       *pool;
    //其中，header_in指向Nginx收到的未经解析的HTTP头部，这里暂不关注它（在第11章中可以看到，header_in就是接收HTTP头部的缓冲区）。 header_in存放请求行，錒eaders_in存放头部行
...
//这是大部分u_char*类型指针对“xxx_start”和“xxx_end”变量的用法。
    ngx_str_t                         uri; //http://10.135.10.167/mytest中的/mytest  http://10.135.10.167/mytest?abc?ttt中的/mytest
    ngx_str_t                         args;//arg指向用户请求中的URL参数。  http://10.135.10.167/mytest?abc?ttt中的abc?ttt
    /*
...
ngx_http_run_posted_requests方法就是通过遍历该单链表来执行子请求的 */ //ngx_http_post_request中创建ngx_http_posted_request_t空间
...
*/
    ...
    //默认是为0的表示需要缓存，如果request_body_no_buffering为1表示不用缓存包体，那么request_body_in_file_only也为0，因为不用缓存包体，那么就不用写到临时文件中
...
阶段开始再次执行，这是Nginx的请求可以反复rewrite重定向的基础。见ngx_http_handler */
    unsigned                          internal:1;//t标志位，为1时表示请求的当前状态是在做内部跳转，则直接
