...
//∞—ngx_http_headers_in÷–µƒÀ˘”–≥…‘±◊ˆhash‘ÀÀ„£¨»ª∫Û¥Ê∑≈µΩcmcf->headers_in_hash÷–
...
µ˜∂» ¬º˛µƒªÿµ˜∑Ω∑®£©£¨’‚ ±ngx_http_init_request∑Ω∑® «‘⁄Õº11-1µƒµ⁄2≤Ω÷–÷¥––µƒ°£µ±»ª£¨‘⁄¥Û≤ø∑÷«Èøˆœ¬£¨ngx_http_init_request∑Ω∑®∫Õ
 ...
 */
    ngx_add_timer(rev, c->listening->post_accept_timeout); //∞—Ω” ’ ¬º˛ÃÌº”µΩ∂® ±∆˜÷–£¨µ±post_accept_timeout√Îªπ√ª”–øÕªß∂À ˝æ›µΩ¿¥£¨æÕπÿ±’¡¨Ω”
    ngx_reusable_connection(c, 1);

    if (ngx_handle_read_event(rev, 0) != NGX_OK) { //µ±œ¬¥Œ”– ˝æ›¥”øÕªß∂À∑¢ÀÕπ˝¿¥µƒ ±∫Ú£¨ª·‘⁄ngx_epoll_process_events∞—∂‘”¶µƒready÷√1°£
        ngx_http_close_connection(c);
        return;
    }
}
...
*/
static void
ngx_http_process_request_line(ngx_event_t *rev) //gx_http_process_request_line∑Ω∑®¿¥Ω” ’HTTP«Î«Û––
{
    ssize_t              n;
    ngx_int_t            rc, rv;
    ngx_str_t            host;
...
// «∑Ò”–«Î«ÛÃÂ“‘º∞«Î«ÛÃÂµƒ≥§∂»£¨»ª∫Û‘Ÿ»•∂¡»°«Î«ÛÃÂ
    for ( ;; ) {
        ...
        /*
         ...
          «∑Òªπ”–ø’œ–µƒƒ⁄¥Ê£¨»Áπ˚ªπ”–Œ¥ π”√µƒƒ⁄¥Êø…“‘ºÃ–¯Ω” ’◊÷∑˚¡˜£¨‘ÚÃ¯◊™µΩµ⁄2≤Ω£¨ºÏ≤Èª∫≥Â«¯ «∑Ò”–Œ¥Ω‚Œˆµƒ◊÷∑˚¡˜£¨∑Ò‘Úµ˜”√
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            //º˚ngx_http_headers_in
            hh = ngx_hash_find(&cmcf->headers_in_hash, h->hash,
                               h->lowcase_key, h->key.len); //ºÏ≤È «∑Ò∏√name“—æ≠¥Ê‘⁄
...
/*
//client_header_timeoutŒ™∂¡»°øÕªß∂À ˝æ› ±ƒ¨»œ∑÷≈‰µƒø’º‰£¨»Áπ˚∏√ø’º‰≤ªπª¥Ê¥¢httpÕ∑≤ø––∫Õ«Î«Û––£¨‘Úª·µ˜”√large_client_header_buffers
 ...
 */
    c->read->handler = ngx_http_request_handler; //”…∂¡–¥ ¬º˛¥•∑¢ngx_http_request_handler
...
/*
ngx_http_request_ handler «HTTP«Î«Û…œ∂¡£Ø–¥ ¬º˛µƒªÿµ˜∑Ω∑®°£‘⁄ngx_event_tΩ·ππÃÂ±Ì æµƒ ¬º˛÷–£¨data≥…‘±÷∏œÚ¡À’‚∏ˆ ¬º˛∂‘”¶µƒngx_connection_t¡¨Ω”£¨
...
”≈œ»¥¶¿Ìø…–¥ ¬º˛ø…“‘æ°øÏ Õ∑≈ƒ⁄”–£¨æ°¡ø±£≥÷∏˜HTTPƒ£øÈ…Ÿ π”√ƒ⁄¥Ê“‘Ã·∏ﬂ≤¢∑¢ƒ‹¡¶£©°£“ÚŒ™∑˛ŒÒ∆˜∑¢ÀÕ∏¯øÕªß∂Àµƒ±®Œƒ≥§∂»“ª∞„±»«Î«Û±®Œƒ¥Û∫‹∂‡
 */
    printf("yang test .....................write:%u\n", ev->write);
    if (ev->write) { //ƒ¨»œ“ª÷±Œ¥1£¨œ¬√Êµƒread∫√œÒ“ª÷±÷¥––≤ª¡À ???????????????????????
...
ngx_http_finalize_connection∑Ω∑®‘⁄Ω· ¯«Î«Û ±£¨Ω‚æˆ¡ÀkeepaliveÃÿ–‘∫Õ◊”«Î«ÛµƒŒ Ã‚
*/
static void
...
discard_bodyŒ™l£¨‘Ú±Ì æ’˝‘⁄∂™∆˙∞¸ÃÂ£¨’‚ ±ª·‘Ÿ“ª¥Œ∞—«Î«Ûµƒread_event_handler≥…‘±…ËŒ™ngx_http_discarded_request_body_handler∑Ω∑®£¨
    ...
    if (r->reading_body) {
        r->keepalive = 0; //∂¡»°øÕªß∂À ˝æ›≥ˆ¥Ì¡À£¨æÕ—”≥Ÿπÿ±’¡¨Ω”
        r->lingering_close = 1;
    }
...
//∞—∂¡ ¬º˛¥”epoll÷–“∆≥˝°£÷ª∂‘epoll ltƒ£ Ω∆‰◊˜”√
//À¸µƒ“‚“Â‘⁄”⁄£¨ƒø«∞“—æ≠ø™ º¥¶¿ÌHTTP«Î«Û£¨≥˝∑«ƒ≥∏ˆHTTPƒ£øÈ÷ÿ–¬…Ë÷√¡Àread_event_handler∑Ω∑®£¨∑Ò‘Ú»Œ∫Œ∂¡ ¬º˛∂ºΩ´µ√≤ªµΩ¥¶¿Ì£¨“≤ø…À∆»œŒ™∂¡ ¬º˛±ª◊Ë »˚¡À°£
void
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
    ...
    ngx_add_timer(rev, clcf->keepalive_timeout); //∆Ù∂Ø±£ªÓ∂® ±∆˜£¨ ±º‰Õ®π˝keepalive_timeout…Ë÷√
...
ngx_http_set_lingering_close(ngx_http_request_t *r)
{
    ...
    ngx_add_timer(rev, clcf->lingering_timeout); //rev->handler = ngx_http_lingering_close_handler;
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
    ...
    ngx_add_timer(rev, timer); //ngx_http_lingering_close_handler
}
...
ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	÷æŒª“≤Œ™0£¨±Ì æ√ª”–HTTPƒ£øÈªπ–Ë“™¥¶¿Ì«Î«Û£¨À˘“‘¥À ±«Î«Ûø…“‘’Ê’˝ Õ∑≈£¨’‚ ±Ã¯µΩµ⁄2≤Ω÷¥––£ª»Áπ˚count“˝”√º∆ ˝¥Û∏…0£¨ªÚ’ﬂblocked¥Û
	”⁄0£¨’‚—˘∂º≤ªø…“‘Ω· ¯«Î«Û£¨ngx_http_close_reques't∑Ω∑®÷±Ω”Ω· ¯°£
     */
...
ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc) // Õ∑≈requestµƒœ‡πÿ◊ ‘¥
{
     ...
     */
    ngx_http_log_request(r); //¥Ú”°http log handler
