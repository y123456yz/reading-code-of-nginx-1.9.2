...
typedef struct {
    ...
    */ //完全匹配keys数组只存放key字符串，节点类型为ngx_str_t，keys_hash hash表存放key对应的key-value中，hash表中数据节点类型为ngx_hash_key_t
    ngx_array_t       keys; //存放所有非通配符key的数组。    //ha->keys中存放的是完全匹配，key  value对
    ngx_array_t      *keys_hash;//完全匹配的字符串key存放在这里面，对应的key-value存放在上面的keys中，判断完全通配符是否重复也是用该hash表判断

    //前置通配符dns_wc_head数组只存放key字符串，节点类型为ngx_str_t，dns_wc_head_hash hash表存放key对应的key-value中，hash表中数据节点类型为ngx_hash_key_t
    ngx_array_t       dns_wc_head;//存放前向通配符key被处理完成以后的值。比如：“*.abc.com”被处理完成以后，变成“com.abc.”被存放在此数组中。
    ngx_array_t      *dns_wc_head_hash;////key对应的key-value对存放在该hash中，同时在存储过程中该值在调用的过程中用来保存和检测是否有冲突的前向通配符的key值，也就是是否有重复。

    //后置通配符dns_wc_tail数组只存放key字符串，节点类型为ngx_str_t，dns_wc_tail_hash hash表存放key对应的key-value中，hash表中数据节点类型为ngx_hash_key_t
    ngx_array_t       dns_wc_tail;//存放后向通配符key被处理完成以后的值。比如：“mail.xxx.*”被处理完成以后，变成“mail.xxx.”被存放在此数组中。
    ngx_array_t      *dns_wc_tail_hash;//key对应的key-value对存放在该hash中，同时在存储过程中该值在调用的过程中用来保存和检测是否有冲突的后向通配符的key值，也就是是否有重复。
} ngx_hash_keys_arrays_t;
