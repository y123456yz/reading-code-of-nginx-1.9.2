...
struct epitem{
    ／／红黑树节点，与第7章中的ngx_rbtree_node七红黑树节点相似
    struct rb node rbn;
...
9.6 epoll事件驱动模块
    本章9.1节～9.5节都在探讨Nginx是如何设计事件驱动框架、如何管理不同的事件驱
...
#define EPOLLERR       0x008 //EPOLLERR|EPOLLHUP都表示连接异常情况  fd用完或者其他吧
...
typedef struct {
     ...
     */
    ngx_uint_t  events; // "epoll_events"参数设置
    ngx_uint_t  aio_requests; // "worker_aio_requests"参数设置
} ngx_epoll_conf_t;
...
{ //一般网络事件中的报文读写通过ngx_handle_read_event  ngx_handle_write_event添加事件
    ...
    /*
    ptr成员存储的是ngx_connection_t连接，可参见9.6.2节中epoll的使用方式。在9.2节中曾经提到过事件的instance标志位，下
    面就配合ngx_epoll_process_events方法说明它的用法
     */
...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)//flags参数中含有NGX_POST_EVENTS表示这批事件要延后处理
{
    ...
    events = epoll_wait(ep, event_list, (int) nevents, timer);  //timer为-1表示无限等待   
        ...
        if ((revents & EPOLLIN) && rev->active) { //如果是读事件且该事件是活跃的
            ...
            if (flags & NGX_POST_EVENTS) {
                /*
                    ...
                    列或者ngx_postedL events队列中。关于post队列中的事件何时执行，可参见9.8节内容
                    */
    ...
    while (ready) {
        ...
        if (events > 0) {
            ...
            for (i = 0; i < events; i++) { //处理event数组里的事件
                ...
                e = (ngx_event_t *) (uintptr_t) event[i].data; //和ngx_file_aio_read io_submit对应
